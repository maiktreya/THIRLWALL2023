...
gdp1 <- rdb("AMECO", "OVGD", dimensions = list(geo = ccode, freq = "a")) %>%
    filter(!is.na(value) & original_period > 1991 & original_period < 2020) # REAL PRICES
gdp <- rdb("AMECO", "UVGD", dimensions = list(geo = ccode, unit = "mrd-ecu-eur", freq = "a")) %>%
...
extra_share <- rdb("AMECO", "DXGE", dimensions = list(geo = ccode, freq = "a")) %>%
    filter(!is.na(value) & original_period > 1991 & original_period < 2020)
eaz <- rdb("AMECO", "UVGD", dimensions = list(geo = "ea12", unit = "mrd-ecu-eur-weighted-mean-of-t-t-1-national-growth-rates-weights-t-1-current-prices-in-ecu-eur", freq = "a")) %>%
    filter(!is.na(value) & original_period > 1991 & original_period < 2020)
usa1 <- rdb("AMECO", "UVGD", dimensions = list(geo = "usa", freq = "a", unit = "mrd-ecu-eur")) %>%
    filter(!is.na(value) & original_period > 1991 & original_period < 2020) # or OVGD [WB/WDI/NY.GDP.MKTP.KD-1W]
jap1 <- rdb("AMECO", "UVGD", dimensions = list(geo = "jpn", freq = "a", unit = "mrd-ecu-eur")) %>%
...
finf <- rdb("AMECO", "PMGS", dimensions = list(geo = ccode, unit = "ecu-eur-2015-100", freq = "a")) %>%
    filter(!is.na(value) & original_period > 1991 & original_period < 2020)
# exports
exp <- rdb("AMECO", "OXGS", dimensions = list(geo = ccode, freq = "a")) %>% # OXGS -> real UXGS -> nom
    filter(!is.na(value) & original_period > 1991 & original_period < 2020)
# imports
imp <- rdb("AMECO", "OMGS", dimensions = list(geo = ccode, freq = "a")) %>% # OMGS -> real UMGS -> nom
    filter(!is.na(value) & original_period > 1991 & original_period < 2020)
# exchange rate
...
eudata1 <- cbind(
    ...
    rpri$value, gdc$value, acc$value, pub_debt$value, gdp1$value, acc1$value, intra_share$value, extra_share$value
)
colnames(eudata1) <- c("period", "target", "income", "prices", "fprices", "exports", "imports", "xrate", "rprices", "consump", "cacc", "debt.pub", "nom_income", "cacc1", "intra", "extra")
for (i in countries) {
...
# save(list = ls(all = TRUE), file = paste0(getwd(), "/RDATA/dbnomics2020.rda"))
eu <- c(
    LfincomeDL[, 1],
    LfincomeDL[, 2],
    LfincomeDL[, 3],
...
eu <- cbind(eudata1[, "target"], "fincome" = eu) %>% data.table::data.table()
write.csv(eu, "Data/CSV/COMTRADE/eudata_fincome_onlyUE_nom.csv")

eud <- data.table::data.table(eudata1)
write.csv(eud, "Data/CSV/COMTRADE/eudata_nom.csv")